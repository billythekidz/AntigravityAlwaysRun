...
(function() {
    ...
    // ==================== CLEANUP ====================
    function cleanup() {
        // Clear all known interval references
        if (window.autoClickerInterval) {
            clearInterval(window.autoClickerInterval);
            console.log('🛑 Stopped previous auto-clicker interval');
        }
        if (window._anticState && window._anticState.interval) {
            clearInterval(window._anticState.interval);
        }
        ...
        document.querySelectorAll('#auto-clicker-indicator').forEach(el => el.remove());
        
        // Clean up references
        window.autoClickerInterval = null;
        window.autoClickerUI = null;
        window._anticState = null;
        
        console.clear();
        console.log('═'.repeat(80));
        console.log('🎯 AUTO-CLICKER FOR "Accept/Run" BUTTON');
        console.log('═'.repeat(80));
    }
    ...
    function findAndClickButton() {
        try {
            ...
            // Helper to scan a document for target buttons
            function scanDocument(doc, sourceIframe) {
                try {
                ...
                } catch (e) { /* Can't access document */ }
            }
            
            // 1. Search the MAIN document first
            ...
            iframes.forEach((iframe) => {
                try {
                    const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                    scanDocument(iframeDoc, iframe);
                } catch (e) { /* Can't access iframe */ }
            });
            
            // 3. Search inside webview elements (Electron/VSCodium)
            const webviews = document.querySelectorAll('webview');
            webviews.forEach((wv) => {
                try {
                    const wvDoc = wv.contentDocument;
                    if (wvDoc) scanDocument(wvDoc, wv);
                } catch (e) { /* Can't access webview */ }
            });
            
            updateUI(foundButtons.length);
            logScanResults(iframes.length + webviews.length, foundButtons.length);
